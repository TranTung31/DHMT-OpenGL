...
// Vẽ tuabine
void draw_frame(mat4 all, float theta) {
	material st = SonTrang;
	draw_shape(vec3(0, 0, 0), vec3(0.8, 14, 0.8), all, st, light_test, 1);
	draw_shape(vec3(0, 7.35, 0), vec3(2.5, 0.7, 0.8), all, st, light_test, 0);
	all = all * Translate(vec3(1.25, 7.35, 0)) * RotateX(theta) * RotateY(180.0);
	draw_shape(vec3(0, 0, 0), vec3(1.5, 0.7, 0.65), all, st, light_test, 3);
}

void draw_wind(mat4 all) {
	material st = SonTrang;
	all = all * RotateY(80);
	draw_shape(vec3(0.0, 0.0, 0.0), vec3(1, 0.75, 0.5), all, st, light_test, 2);
}

void draw_tuabine(vec3 T, float theta) {
	mat4 instance;
	instance = Translate(T) * Scale(2.0, 2.0, 2.0);
	draw_frame(instance, theta);
	instance = instance * Translate(1.5, 7.35, 0) * RotateX(theta);
	draw_wind(instance);
